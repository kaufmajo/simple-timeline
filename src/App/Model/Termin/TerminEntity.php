...
class TerminEntity extends AbstractEntity implements TerminEntityInterface
{
    ...
    protected ?string $termin_label = null;
    ...
    public function initBulk(): static
    {
        foreach ($this->fields as $field) {
            $rp           = new ReflectionProperty($this, $field);
            $this->$field = match ($field) {
                'termin_zeit_start', 'termin_zeit_ende' => '00:00',
                'termin_serie_intervall', 'termin_serie_wiederholung', 'termin_serie_ende' => null,
                default => match ($rp->getType()->getName()) {
                    'int' => IGNORE_INT_VALUE,
                    'string' => IGNORE_STRING_VALUE,
                }
            };
        }
    ...
    {
        return $this->termin_label;
    }

    public function setTerminLabel(?string $value): void
    {
        $this->termin_label = $value;
    }
