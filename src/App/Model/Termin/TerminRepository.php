...
class TerminRepository extends AbstractRepository implements TerminRepositoryInterface
{
    ...
    protected function getTerminJoinCondition(array $params = []): Where
    {
        ...
        if (! empty($params['suchtext'])) {
            $whereLikeSearch = $this->dbRunner->whereLikeSearchWithSqlObject($params['suchtext'], [
                ...
                ['column' => 't4.termin_label', 'search' => '%[value]%'],
    ...
    public function fetchTermin(array $params = [], array $groupBy = ['t3.datum_id', 't4.termin_id'], string $order = 't3.datum_datum ASC, t4.termin_zeit_start ASC, t4.termin_zeit_ende ASC, t4.termin_sortierung ASC'): ResultSet
    {
        ...
        $select->join(
            ...
            [
                ...
                '_def_neu_infobox'               => new Sql\Expression('IF(? > DATEDIFF(CURDATE(), termin_datum_start), 1, 0)', $initConfig['considered_def_neu_infobox']),
                '_def_neu'                       => new Sql\Expression('IF(? > DATEDIFF(CURDATE(), termin_erstellt_am), 1, 0)', $initConfig['considered_def_new']),
                '_mng_neu'                       => new Sql\Expression('IF(? > DATEDIFF(CURDATE(), termin_erstellt_am), 1, 0)', $initConfig['considered_mng_new']),
                '_def_updated_infobox'           => new Sql\Expression('IF(? > DATEDIFF(CURDATE(), termin_aktualisiert_am_trigger), 1, 0)', $initConfig['considered_def_updated_infobox']),
                '_def_updated'                   => new Sql\Expression('IF(? > DATEDIFF(CURDATE(), termin_aktualisiert_am_trigger), 1, 0)', $initConfig['considered_def_updated']),
                '_mng_updated'                   => new Sql\Expression('IF(? > DATEDIFF(CURDATE(), termin_aktualisiert_am_trigger), 1, 0)', $initConfig['considered_mng_updated']),
...
//        ddd($select->getSqlString($this->dbRunner->getDb()->platform));
    ...
    public function fetchLabel(array $params = [], string $order = 'termin_label ASC'): ResultSet
    {
        $sql    = new Sql\Sql($this->dbRunner->getDb());
        $select = $sql->select(['t3' => 'tajo1_datum']);
        $select->columns([]);
        ...
        $select->group('t4.termin_label');
        $select->order($order);
        $select->where($this->getWhereCondition($params));
        $select->where
            ->isNotNull('t4.termin_label')
            ->notEqualTo('t4.termin_label', '');
