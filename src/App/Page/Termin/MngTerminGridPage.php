...
class MngTerminGridPage extends AbstractBasePage
{
    ...
    public function indexAction(ServerRequestInterface $request): HtmlResponse
    {
        ...
        return new HtmlResponse(
            $this->templateRenderer->render('app::mng/termin/mng-termin-grid-index', $viewData)
        );
    }
    ...
    public function getTerminSearchForm(array $params): FormInterface
    {
        ...
        $form->setAttribute('action', '/manage/termin-grid');
    ...
    public function getMappedTerminSearchValues(string $date): array
    {
        ...
        $searchValues              = [];
        $searchValues['start']     = (new DateTime($date))->modify('first day of this month')->format('Y-m-d');
        $searchValues['ende']      = (new DateTime($date))->modify('last day of this month')->format('Y-m-d');
