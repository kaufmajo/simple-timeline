...
populateSelectIntervall(inputWiederholung, inputIntervall.value);
...
function populateSelectIntervall(inputSelect, intervall) {
    // init
    const options = inputSelect.querySelectorAll("option");
    //
    for (let i = 0, len = options.length; i < len; i++) {
        //
        if (options[i].dataset.intervall === intervall || options[i].value === '') {
